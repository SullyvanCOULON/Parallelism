...
int convolution(Dimension dimension, char *filename, char *file_out) {
    ...
    for (int k = 0; k < N; k++) {
        ...
        #pragma omp parallel for schedule(static) collapse(2) firstprivate(d2x, d2y) private(nn, sn, wn, en, cp, F) 
        for (int i = 0; i < cl; i++) { 
            for (int j = 0; j < rw; j++) {
    ...
    printf("One parallel execution finished.\tExecution time: %f\n", (end_time - start_time)); 
...
int main(int argc, char* argv[]) {
    ...
    if (argc < 3) {
        ...
        printf("Example: %s images/papoch.pgm images/image_out_par.pgm 1D\n", argv[0]);
        return (EXIT_FAILURE);
